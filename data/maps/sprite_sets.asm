; Valid sprite IDs for each outdoor map.

MapSpriteSets:
	table_width 1, MapSpriteSets
	db SPRITESET_PALLET_VIRIDIAN ; PALLET_TOWN
	db SPRITESET_PALLET_VIRIDIAN ; VIRIDIAN_CITY
	db SPRITESET_PEWTER_CERULEAN ; PEWTER_CITY
	db SPRITESET_PEWTER_CERULEAN ; CERULEAN_CITY
	db SPRITESET_VERMILION       ; VERMILION_CITY
	db SPRITESET_CELADON         ; CELADON_CITY
	db SPRITESET_LAVENDER        ; LAVENDER_TOWN
	db SPRITESET_SAFFRON         ; SAFFRON_CITY
	db SPRITESET_FUCHSIA         ; FUCHSIA_CITY
	db SPRITESET_INDIGO          ; RANGER_STATION
	db SPRITESET_PALLET_VIRIDIAN ; CINNABAR_ISLAND
	db SPRITESET_INDIGO          ; INDIGO_PLATEAU
	db SPRITESET_PALLET_VIRIDIAN ; ROUTE_1
	db SPLITSET_ROUTE_2          ; ROUTE_2
	db SPRITESET_PALLET_VIRIDIAN ; ROUTE_3
	db SPRITESET_PALLET_VIRIDIAN ; ROUTE_4
	db SPLITSET_ROUTE_5          ; ROUTE_5
	db SPLITSET_ROUTE_6          ; ROUTE_6
	db SPLITSET_ROUTE_7          ; ROUTE_7
	db SPLITSET_ROUTE_8          ; ROUTE_8
	db SPRITESET_X_ISLE ; ROUTE_9
	db SPLITSET_ROUTE_10         ; ROUTE_10
	db SPRITESET_RANGER_EXTERIOR        ; ROUTE_11
	db SPLITSET_ROUTE_12         ; ROUTE_12
	db SPRITESET_SILENCE_BRIDGE  ; ROUTE_13
	db SPRITESET_SILENCE_BRIDGE  ; ROUTE_14
	db SPLITSET_ROUTE_15         ; ROUTE_15
	db SPLITSET_ROUTE_16         ; ROUTE_16
	db SPRITESET_CYCLING_ROAD    ; ROUTE_17
	db SPLITSET_ROUTE_18         ; ROUTE_18
	db SPRITESET_PALLET_VIRIDIAN ; ROUTE_19
	db SPLITSET_ROUTE_20         ; ROUTE_20
	db SPRITESET_KUNZITE_FISHING_AREA; ROUTE_21
	db SPRITESET_PALLET_VIRIDIAN ; ROUTE_22
	db SPRITESET_INDIGO          ; ROUTE_23
	db SPRITESET_TRAILER_PARK    ; ROUTE_24
	db SPRITESET_FELDSPAR_FLATS ; ROUTE_25
	assert_table_length FIRST_INDOOR_MAP

; Format:
; #1: whether the map is split EAST_WEST or NORTH_SOUTH
; #2: coordinate of dividing line
; #3: sprite set ID if on the west or north side
; #4: sprite set ID if on the east or south side
SplitMapSpriteSets:
	table_width 4, SplitMapSpriteSets
	db EAST_WEST, 37, SPRITESET_PALLET_VIRIDIAN, SPRITESET_PEWTER_CERULEAN ; SPLITSET_ROUTE_2
	db NORTH_SOUTH, 50, SPRITESET_PEWTER_CERULEAN, SPRITESET_LAVENDER        ; SPLITSET_ROUTE_10
	db EAST_WEST,   57, SPRITESET_VERMILION,       SPRITESET_SILENCE_BRIDGE  ; SPLITSET_ROUTE_11
	db EAST_WEST,  3, SPRITESET_LAVENDER,        SPRITESET_SILENCE_BRIDGE  ; SPLITSET_ROUTE_12
	db EAST_WEST,    8, SPRITESET_FUCHSIA,         SPRITESET_SILENCE_BRIDGE  ; SPLITSET_ROUTE_15
	db NORTH_SOUTH,   40, SPRITESET_CYCLING_ROAD,    SPRITESET_CELADON         ; SPLITSET_ROUTE_16
	db NORTH_SOUTH,   40, SPRITESET_CYCLING_ROAD,    SPRITESET_FUCHSIA         ; SPLITSET_ROUTE_18
	db EAST_WEST,   53, SPRITESET_PALLET_VIRIDIAN, SPRITESET_FUCHSIA         ; SPLITSET_ROUTE_20
	db NORTH_SOUTH, 56, SPRITESET_PEWTER_CERULEAN, SPRITESET_SAFFRON         ; SPLITSET_ROUTE_5
	db NORTH_SOUTH,  2, SPRITESET_SAFFRON,         SPRITESET_VERMILION       ; SPLITSET_ROUTE_6
	db EAST_WEST,   17, SPRITESET_LAVENDER,        SPRITESET_SAFFRON         ; SPLITSET_ROUTE_7
	db EAST_WEST,    3, SPRITESET_SAFFRON,         SPRITESET_LAVENDER        ; SPLITSET_ROUTE_8
	assert_table_length NUM_SPLIT_SETS

SpriteSets:
	table_width SPRITE_SET_LENGTH, SpriteSets

; SPRITESET_PALLET_VIRIDIAN
	db SPRITE_BLUE
	db SPRITE_YOUNGSTER
	db SPRITE_GIRL
	db SPRITE_FISHER
	db SPRITE_JUSTIN
	db SPRITE_GAMBLER
	db SPRITE_COOLTRAINER_F
	db SPRITE_OAK
	db SPRITE_SWIMMER
	db SPRITE_POKE_BALL
	db SPRITE_GAMBLER_ASLEEP

; SPRITESET_PEWTER_CERULEAN
	db SPRITE_YOUNGSTER
	db SPRITE_LISBETH
	db SPRITE_SCIENTIST
	db SPRITE_HIKER
	db SPRITE_LAWNCRETE
	db SPRITE_BLUE
	db SPRITE_GUARD
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

; SPRITESET_LAVENDER
	db SPRITE_LITTLE_GIRL
	db SPRITE_ERIKA
	db SPRITE_SUPER_NERD
	db SPRITE_YOUNGSTER
	db SPRITE_GAMBLER
	db SPRITE_MONSTER
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_GUARD
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

; SPRITESET_VERMILION
	db SPRITE_BEAUTY
	db SPRITE_FISHER
	db SPRITE_YOUNGSTER
	db SPRITE_GAMBLER
	db SPRITE_FAIRY
	db SPRITE_ERIKA
	db SPRITE_SAILOR
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

; SPRITESET_CELADON
	db SPRITE_LITTLE_GIRL
	db SPRITE_LITTLE_BOY
	db SPRITE_GIRL
	db SPRITE_MBIKER
	db SPRITE_CUE_BALL
	db SPRITE_GRAMPALT
	db SPRITE_MONSTER
	db SPRITE_GUARD
	db SPRITE_FISHER
	db SPRITE_POKE_BALL
	db SPRITE_SNORLAX

; SPRITESET_INDIGO
	db SPRITE_YOUNGSTER
	db SPRITE_GYM_GUIDE
	db SPRITE_MONSTER
	db SPRITE_BLUE
	db SPRITE_COOLTRAINER_F
	db SPRITE_COOLTRAINER_M
	db SPRITE_SWIMMER
	db SPRITE_GUARD
	db SPRITE_GAMBLER
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

; SPRITESET_SAFFRON
	db SPRITE_ROCKET
	db SPRITE_SCIENTIST
	db SPRITE_SILPH_WORKER
	db SPRITE_ERIKA
	db SPRITE_GENTLEMAN
	db SPRITE_BIRD
	db SPRITE_ROCKER
	db SPRITE_COOLTRAINER_M
	db SPRITE_MONSTER
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

; SPRITESET_SILENCE_BRIDGE
	db SPRITE_BIKER
	db SPRITE_SUPER_NERD
	db SPRITE_CUE_BALL
	db SPRITE_COOLTRAINER_F
	db SPRITE_OFFICER
	db SPRITE_BEAUTY
	db SPRITE_FISHER
	db SPRITE_LORELEI
	db SPRITE_SAILOR
	db SPRITE_POKE_BALL
	db SPRITE_SNORLAX

; SPRITESET_CYCLING_ROAD
	db SPRITE_BIKER
	db SPRITE_PSYCHIC
	db SPRITE_OFFICER
	db SPRITE_FISHER
	db SPRITE_HEALER
	db SPRITE_HIKER
	db SPRITE_GAMBLER
	db SPRITE_CUE_BALL
	db SPRITE_MBIKER
	db SPRITE_POKE_BALL
	db SPRITE_SNORLAX

; SPRITESET_FUCHSIA
	db SPRITE_BIRD
	db SPRITE_COOLTRAINER_M
	db SPRITE_FAIRY
	db SPRITE_FISHER
	db SPRITE_OFFICER
	db SPRITE_MONSTER
	db SPRITE_SEEL
	db SPRITE_PSYCHIC
	db SPRITE_YOUNGSTER
	db SPRITE_POKE_BALL
	db SPRITE_FOSSIL
	
; SPRITESET_TRAILER_PARK
	db SPRITE_BIKER
	db SPRITE_LAWNCRETE
	db SPRITE_SILPH_PRESIDENT
	db SPRITE_FISHER
	db SPRITE_GRANNY
	db SPRITE_HIKER
	db SPRITE_GAMBLER
	db SPRITE_CUE_BALL
	db SPRITE_MBIKER
	db SPRITE_POKE_BALL
	db SPRITE_SNORLAX
	
; SPRITESET_FELDSPAR_FLATS
	db SPRITE_GIRL
	db SPRITE_CHANNELER
	db SPRITE_DAISY
	db SPRITE_ERIKA
	db SPRITE_GRAMPS
	db SPRITE_HIKER
	db SPRITE_GAMBLER
	db SPRITE_ROCKER
	db SPRITE_PSYCHIC
	db SPRITE_POKE_BALL
	db SPRITE_BOULDER
	
; SPRITESET_X_ISLE
	db SPRITE_YOUNGSTER
	db SPRITE_LISBETH
	db SPRITE_SCIENTIST
	db SPRITE_HIKER
	db SPRITE_LAWNCRETE
	db SPRITE_KOGA
	db SPRITE_COOLTRAINER_M
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2
	
; SPRITESET_RANGER_EXTERIOR
	db SPRITE_GAMBLER
	db SPRITE_FAIRY
	db SPRITE_YOUNGSTER
	db SPRITE_FISHER
	db SPRITE_ERIKA
	db SPRITE_KOGA
	db SPRITE_COOLTRAINER_M
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2
	
; SPRITESET_KUNZITE_FISHING_AREA
	db SPRITE_CUE_BALL
	db SPRITE_SWIMMER
	db SPRITE_FISHING_GURU
	db SPRITE_FISHER
	db SPRITE_ERIKA
	db SPRITE_KOGA
	db SPRITE_COOLTRAINER_M
	db SPRITE_COOLTRAINER_F
	db SPRITE_JUSTIN
	db SPRITE_POKE_BALL
	db SPRITE_UNUSED_GAMBLER_ASLEEP_2

	assert_table_length NUM_SPRITE_SETS
