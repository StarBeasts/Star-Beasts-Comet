MACRO hidden_item
	db \1, \3, \2
ENDM

HiddenItemCoords:
	; map id, x, y
	hidden_item VIRIDIAN_FOREST,               9,  31
	hidden_item VIRIDIAN_FOREST,               9,  17
	hidden_item MT_MOON_B2F,                   39,  19
	hidden_item ROUTE_25,                      2,   4
	hidden_item ROUTE_9,                       22,   8
	hidden_item SS_ANNE_KITCHEN,               13,   9
	hidden_item SS_ANNE_B1F_ROOMS,              3,   1
	hidden_item ROUTE_10,                      12,  23
	hidden_item ROUTE_10,                      13,  18
	hidden_item ROCKET_HIDEOUT_B1F,            16,  14
	hidden_item ROCKET_HIDEOUT_B3F,            4,  22
	hidden_item ROCKET_HIDEOUT_B4F,            3,   13
	hidden_item POKEMON_TOWER_5F,               9,  2
	hidden_item ROUTE_13,                      19,  36
	hidden_item ROUTE_13,                      40,  25
	hidden_item POKEMON_MANSION_B1F,            3,  18
	hidden_item SAFARI_ZONE_GATE,              0,   6
	hidden_item SAFARI_ZONE_WEST,               6,   5
	hidden_item SILPH_CO_5F,                   30,   2
	hidden_item SILPH_CO_9F,                    24,  13
	hidden_item COPYCATS_HOUSE_2F,              6,   7
	hidden_item CERULEAN_CAVE_1F,              14,  11 ; later
	hidden_item CERULEAN_CAVE_B1F,             27,   3 ; later
	hidden_item POWER_PLANT,                   22,  24
	hidden_item POWER_PLANT,                   35,   21
	hidden_item SEAFOAM_ISLANDS_B2F,           32,  9
	hidden_item SEAFOAM_ISLANDS_B4F,           23,  25
	hidden_item POKEMON_MANSION_1F,             2,  4
	hidden_item POKEMON_MANSION_3F,             18,   1
	hidden_item ROUTE_23,                       21,  94
	hidden_item ROUTE_23,                       4,  85
	hidden_item ROUTE_23,                       2,  53
	hidden_item VICTORY_ROAD_2F,                7,  13
	hidden_item VICTORY_ROAD_2F,               45,  12
	hidden_item VILLA_1F,      	           9,    2
	hidden_item VIRIDIAN_CITY,                 36,   2
	hidden_item ROUTE_11,                      33,  19
	hidden_item ROUTE_12,                      31,  9
	hidden_item ROUTE_17,                      16,  48
	hidden_item ROUTE_17,                      14,  64
	hidden_item ROUTE_17,                      19,  64
	hidden_item ROUTE_17,                      16,  79
	hidden_item ROUTE_17,                      20, 90
	hidden_item UNDERGROUND_PATH_NORTH_SOUTH,   5,  3
	hidden_item UNDERGROUND_PATH_NORTH_SOUTH,  18,  17
	hidden_item UNDERGROUND_PATH_WEST_EAST,    18,   2
	hidden_item UNDERGROUND_PATH_WEST_EAST,    18,  14
	hidden_item CELADON_CITY,                  6,  4
	hidden_item ROUTE_25,                      26,   8
	hidden_item MT_MOON_B2F,                   15,   30
	hidden_item SEAFOAM_ISLANDS_B3F,           26,  8
	hidden_item VERMILION_CITY,                10,  3
	hidden_item CERULEAN_CITY,                 11,   6
	hidden_item ROUTE_4,                       16,   2
	hidden_item ROUTE_25,                      33,   16
	db -1 ; end
